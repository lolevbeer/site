...
export const migrationRecovery = [
  {
    ...
    retry: 'Rerun only as a roll-forward: review upserts are keyed by source URL, and the immutable down migration is not production recovery.',
    mode: 'roll-forward',
    verify: 'Expected count is distinct legacy reviews with both URL and text; each URL must retain its source beer association, and skipped or cross-beer URL keys must be enumerated.',
  },
  {
    ...
    retry: 'Before retrying, record normalized schedule and exclusion counts at or below 10,000 and investigate duplicate keys; otherwise use a reviewed roll-forward.',
    mode: 'roll-forward',
    verify: 'For populated schedule slots and valid exclusion dates, matched normalized counts must equal distinct legacy keys; enumerate skipped keys and confirm normalizedAt is set.',
  },
  {
    ...
    retry: 'Inspect both named indexes first; retry is safe only when each target is absent or already has its exact required key and options.',
    mode: 'roll-forward',
    verify: 'Read-only inspect payload_jobs_runnable (queue, processing, hasError, completedAt, waitUntil, createdAt) and payload_jobs_schedule_dedupe (taskSlug, queue, completedAt, createdAt), both with name-only options; observe the next scheduled maintenance run.',
  },
  {
